...
export class LnHospitalSpacesList {
  ...
  private openEditDialog(id: string) {
    ...
    this.spaceToEdit = foundSpace ? { ...foundSpace } : null;
    this.isEditorOpen = true;
  }

  private handleSave(ev: CustomEvent<HospitalSpace>) {
    const savedData = ev.detail;
    
    if (savedData.id.includes('temp')) {
      savedData.id = `space-${Math.random().toString(36).substr(2, 9)}`;
      this.spaces = [savedData, ...this.spaces];
    } else {
      this.spaces = this.spaces.map(s => s.id === savedData.id ? savedData : s);
    }
    this.isEditorOpen = false;
  }

  private handleDeleteSpace(id: string) {
    console.log(`Mazanie priestoru s ID: ${id}`);
    if (confirm('Naozaj chcete vymazať tento priestor z evidencie?')) {
  ...
  private getGroupedSpaces() {
    const grouped: { [pavilion: string]: { [floor: number]: HospitalSpace[] } } = {};
    ...
    return (
      <div class="spaces-wrapper" style={{ padding: '24px', fontFamily: 'var(--md-sys-typescale-body-large-font, sans-serif)' }}>
        ...
        <ln-hospital-spaces-edit
          ...
          onEditor-saved={(ev) => this.handleSave(ev)}      
        ></ln-hospital-spaces-edit>
      </div>
    );
  }
